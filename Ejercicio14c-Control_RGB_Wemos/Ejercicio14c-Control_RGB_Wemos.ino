...
byte led = 1; //Led que enciendo para efecto
boolean anterior_b;
...
void loop() {
    ...
    if (estado_b == LOW) {  //flanco ascendente pull-up
      ...
      if (duracion_pulso < 2000) {  //corta
        ...
        if (encendido) {
          ...
          leds.show();
          tiempo_led = millis();
          led = 0;  //empiezo el efecto en el led 1
        }
      }
      else {
  ...
  if (encendido) {
    if ((millis() - tiempo_led) > 200) {
      tiempo_led = millis();
      leds.setPixelColor(((led + (LED_NUM - 2)) % (LED_NUM - 1)) + 1, leds.Color(0, 0, 0));
      leds.setPixelColor((led % (LED_NUM - 1)) + 1, leds.Color(colores[0], colores[1], colores[2]));
      leds.show();
      led++;
      if (led > LED_NUM - 2)
        led = 0;
